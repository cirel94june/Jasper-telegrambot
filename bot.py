...
def build_group_identity_hint(chat_id):
    ...
    for uid, raw in items:
        ...
        if item.get("is_bot") and linked_human_id:
            suffix += (
                f"，关联的群友是{_identity_display_name(chat_id, linked_human_id)}"
                f"（Telegram用户数字 {linked_human_id}）"
            )
...
def describe_message_identity(chat_id, msg):
    ...
    if linked_human_id:
        lines.append(
            f"关联群友：{_identity_display_name(chat_id, linked_human_id)} "
            f"(ID:{linked_human_id})"
        )
    elif target_is_bot:
...
def webhook():
    ...
    # 每条消息都按 Telegram 数字 ID 学身份；显示名只作为别名，不能反推人。
    ...
    if chat_id.startswith("-") and replied_for_identity and can_teach_identity:
        ...
        if target_id:
            observe_identity(
                chat_id, target_id, target_name, target_username, target_is_bot
            )
        relationship = _extract_identity_relationship(user_text, target_is_bot)
        if relationship:
            identity_alias_taught = True
            if target_is_bot:
                human_id = _resolve_identity_reference(
                    chat_id, relationship.get("human_ref"), expected_is_bot=False
                )
                if human_id:
                    learn_identity_relationship(
                        chat_id,
                        target_id,
                        human_id,
                        learned_by=sender_id,
                        bot_alias=relationship.get("bot_alias", ""),
                    )
                else:
                    print(
                        f"[IDENTITY] relationship unresolved chat={chat_id} "
                        f"human={relationship.get('human_ref')!r}"
                    )
            else:
                bot_id = _resolve_identity_reference(
                    chat_id, relationship.get("bot_ref"), expected_is_bot=True
                )
                if bot_id:
                    learn_identity_relationship(
                        chat_id, bot_id, target_id, learned_by=sender_id
                    )
                else:
                    print(
                        f"[IDENTITY] relationship unresolved chat={chat_id} "
                        f"bot={relationship.get('bot_ref')!r}"
                    )
        else:
    ...
    if command_name == "/whois" and chat_id.startswith("-"):
        ...
        send_telegram(
            chat_id,
            "这条消息的真实发送身份：\n" + describe_message_identity(chat_id, replied),
            reply_to_message_id=msg.get("message_id"),
        )
        return "ok"
    ...
    if user_text.strip().lower().startswith("/checkuser"):
        ...
        if len(parts) >= 2:
            ...
            try:
                ...
                info = (f"status={member.get('status', '?')}  "
                        f"title={member.get('custom_title', '')}  "
                        f"is_bot={(member.get('user') or {}).get('is_bot', '?')}")
