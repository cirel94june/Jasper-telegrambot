...
class ConversationContinuityTest(unittest.TestCase):
    def test_image_keeps_all_preceding_speakers(self):
        history = [
            {"role": "user", "raw_text": "blue marble", "stable_sender_id": "ceci"},
            {"role": "user", "raw_text": "[image]", "stable_sender_id": "user:42"},
        ]
        dialogue = bot.build_model_messages(history)[-1]["content"]
        blocks = bot._attach_current_images(dialogue, [
            {"type": "_bot_image", "data": "test", "media_type": "image/jpeg"},
            {"type": "text", "text": "[image]"},
        ])
        self.assertIn("blue marble", blocks[-1]["text"])
        self.assertIn("[image]", blocks[-1]["text"])
        self.assertEqual(blocks[0]["data"], "test")
